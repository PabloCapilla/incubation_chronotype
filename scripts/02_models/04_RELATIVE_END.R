...
anova(model_relative_end, 
      update(model_relative_end, 
             . ~ . - area:poly(inc_start_aprildays, 2)[, 2]),
      test = "LRT")
...
full_model <- update(model_relative_end, 
                     . ~ . 
                     - area:poly(inc_start_aprildays, 2)[, 2] 
                     - area:poly(inc_start_aprildays, 2)[, 1]
                     - area:poly(day_before_hatch, 2)[, 2])
...
rep_relative_end <- rpt(activity_end_relative ~ 
                          area:poly(day_before_hatch,2)[,1] +  
...
## base table
table_relative_end00 <- full_model %>%
...
## add features
table_relative_end <- table_relative_end00 %>% 
...
full_model_predictions <- lmer(activity_end_relative ~ 
